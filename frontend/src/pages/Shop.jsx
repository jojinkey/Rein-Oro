...
export default function Shop() {
 ...
 useSEO({
  ...
  description:
   "Explore the full catalog of royal health snacks, including slow-roasted Flavored Makhanas, Premium Cashews, California Almonds, and Luxury Gift Hampers.",
 ...
 useEffect(() => {
  ...
  if (categoryParam !== "All") {
   result = result.filter(
    (p) =>
     p.name.toLowerCase() === categoryParam.toLowerCase() ||
     p.id.includes(categoryParam.toLowerCase()),
   );
  }
  ...
  if (selectedFlavors.length > 0) {
   result = result.filter((p) => {
    return selectedFlavors.some((f) =>
     p.flavor.toLowerCase().includes(f.toLowerCase()),
    );
   });
  }
  ...
  <main className="cart-page-main">
   <div className="shop-layout-container">
        ...
        <li key={cat}>
         <button
          ...
          style={{
           ...
           border: "none",
           color:
            categoryParam === cat ? "var(--color-gold)" : "var(--color-muted)",
           cursor: "pointer",
           fontSize: "0.85rem",
    ...
    <section className="products-catalog">
     ...
     >
      <span className="section-subtitle" style={{ fontSize: "0.72rem" }}>
       Royal Selection
      </span>
