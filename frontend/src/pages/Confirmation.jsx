...
export default function Confirmation() {
  ...
  useEffect(() => {
    fetch('/api/products')
      .then(res => res.json())
      .then(data => {
        const orderItemIds = order && order.items ? order.items.map(item => item.id) : [];
    ...
    <main className="cart-page-main">
      <div className="confirmation-layout">
        ...
        <section className="thank-you-header">
          ...
          <h1>Thank You!</h1>
          <p>Your order has been placed successfully. A receipt has been sent to your email.</p>
        ...
        <section className="confirmation-columns-grid">
          ...
          <div className="receipt-col-card">
            ...
            <div className="summary-footer">
              ...
              <div className="summary-row">
                ...
                <span className="value">{order.shipping === 0 ? 'Free' : `₹${order.shipping}`}</span>
              </div>
              <div className="summary-row">
          ...
          <div className="receipt-col-card highlight-card">
            ...
            <p className="brand-desc">
              Crafted with love, delivered with care. Your snacks are prepared inside highly sanitized, vacuum-sealed chambers and shipped securely to retain maximum crunch. Thank you for choosing Rein Oro.
            </p>
          </div>
            ...
            {[
              ...
              <div key={idx} className={`timeline-step ${step.active ? 'active' : ''}`}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <h4 className="step-label" style={{ margin: 0 }}>{step.label}</h4>
                  <p className="step-desc" style={{ margin: '2px 0 0 0' }}>{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {products.length > 0 && (
          <section className="confirmation-upsell-section" style={{ marginTop: '4rem', borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: '3rem' }}>
            <h2 className="timeline-heading" style={{ marginBottom: '0.5rem', textAlign: 'center' }}>Complete Your Collection</h2>
            <p style={{ color: 'var(--color-muted)', fontSize: '0.9rem', marginBottom: '2rem', textAlign: 'center' }}>
              Add these royal snacks to your next delivery for an exclusive taste.
            </p>
            {addedMessage && (
              <div style={{ backgroundColor: 'rgba(201,168,76,0.1)', border: '1px solid var(--color-gold)', color: 'var(--color-white)', padding: '1rem', borderRadius: '4px', textAlign: 'center', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
                {addedMessage}
              </div>
            )}
            <div className="products-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
              {products.map(p => (
                <div key={p.id} className="product-card" style={{ border: '1px solid rgba(201,168,76,0.12)', borderRadius: '4px', overflow: 'hidden', backgroundColor: '#141414', transition: 'transform 0.3s' }}>
                  <div style={{ height: '220px', backgroundColor: '#1C1A16', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
