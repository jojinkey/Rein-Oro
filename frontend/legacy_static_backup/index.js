...
if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
    ...
    console.error("GSAP or ScrollTrigger is not loaded. Scroll animations will not run.");
}
    ...
    if (typeof gsap !== "undefined") {
        ...
        if (window.scrollY < 50) {
            window.scene1Entrance = gsap.to("#scene-1", 
                { autoAlpha: 1, y: 0, filter: "blur(0px)", duration: 1.8, ease: "power3.out", delay: 0.4 }
            );
        } else {
...
function buildScrollAnimations() {
    if (typeof gsap === "undefined" || typeof ScrollTrigger === "undefined") {
        console.warn("GSAP / ScrollTrigger not loaded. Skipping scroll animation setup.");
        return;
    }
        ...
        scrollTrigger: {
            ...
            onUpdate: (self) => {
                ...
                if (self.progress > 0.01) {
                    ...
                    gsap.set("#scene-1", { autoAlpha: 1, y: 0, filter: "blur(0px)", overwrite: "auto" });
                }
...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        // Reset progress bar
        if (trackerText) trackerText.innerHTML = "You are <strong>₹999</strong> away from FREE shipping!";
    ...
    if (diff <= 0) {
        if (trackerText) trackerText.innerHTML = "You have unlocked <strong>FREE shipping!</strong>";
        if (progressBar) progressBar.style.width = "100%";
        if (progressTruck) progressTruck.style.left = "100%";
    } else {
        const percent = Math.min(100, Math.round((subtotal / limit) * 100));
        if (trackerText) trackerText.innerHTML = `You are <strong>₹${diff}</strong> away from FREE shipping!`;
        ...
        btn.addEventListener("click", (e) => {
            ...
            if (!productId) {
                ...
                if (img) {
                    ...
                    if (src.includes("makhana_cheese_onion")) productId = "makhana_cheese_onion";
                    ...
                    else if (src.includes("makhana_himalayan")) productId = "makhana_himalayan";
                    else if (src.includes("almonds_california")) productId = "almonds_california";
                    else if (src.includes("cashews_roasted")) productId = "cashews_roasted";
                    else if (src.includes("pistachios_roasted")) productId = "pistachios_roasted";
            ...
            if (productId) {
                ...
                if (typeof gsap !== "undefined") {
                    gsap.fromTo(btn,
                        { scale: 1 },
                        { scale: 0.82, duration: 0.1, yoyo: true, repeat: 1, ease: "power1.inOut" }
                    );
                }
            }
        });
...
function renderCartPage() {
            ...
            <div class="cart-page-item-product">
                ...
                <div class="cart-page-item-details">
                    ...
                    <div class="cart-page-item-badges">
                        ${benefits.slice(0, 2).map(b => `<span class="cart-page-item-badge">${b}</span>`).join('')}
                    </div>
    ...
    rowsContainer.querySelectorAll(".btn-page-remove-item-cross").forEach(btn => {
        btn.addEventListener("click", () => removeHandler(btn));
    });
...
function setupCheckoutPage() {
    ...
    document.querySelectorAll('input[name="deliveryMethod"]').forEach(radio => {
        ...
        radio.addEventListener('change', () => {
            document.querySelectorAll('.delivery-select-card').forEach(card => card.classList.remove('active'));
            radio.closest('.delivery-select-card').classList.add('active');
    ...
    document.querySelectorAll('input[name="paymentMethod"]').forEach(radio => {
        ...
        radio.addEventListener('change', () => {
            document.querySelectorAll('.payment-select-card').forEach(card => card.classList.remove('active'));
            radio.closest('.payment-select-card').classList.add('active');
            renderCheckoutSummary();
        });
    });
    ...
    if (savedAddress) {
        try {
            ...
            if (document.getElementById("phoneNumber")) document.getElementById("phoneNumber").value = addr.phoneNumber || "";
            if (document.getElementById("emailAddress")) document.getElementById("emailAddress").value = addr.emailAddress || "";
            if (document.getElementById("address")) document.getElementById("address").value = addr.address || "";
            if (document.getElementById("apartment")) document.getElementById("apartment").value = addr.apartment || "";
            ...
            if (document.getElementById("saveAddress")) document.getElementById("saveAddress").checked = true;
        } catch (e) {
    ...
    form.addEventListener("submit", (e) => {
        ...
        form.querySelectorAll(".form-input").forEach(inp => inp.classList.remove("is-invalid"));
        const termsGroup = form.querySelector(".checkout-terms-agreement");
        if (termsGroup) termsGroup.classList.remove("is-invalid");
        
        // Validate required fields
        const requiredFields = ["fullName", "phoneNumber", "emailAddress", "address", "city", "state", "pinCode"];
        ...
        if (phone && phone.value.trim() && !/^[+0-9\s-]{10,15}$/.test(phone.value.trim())) {
        ...
        if (saveAddressChecked) {
            ...
            localStorage.setItem("rein_oro_checkout_address", JSON.stringify(addressData));
        } else {
        ...
        if (placeOrderBtn) {
            ...
            form.querySelectorAll(".form-input, input[type='radio'], input[type='checkbox']").forEach(control => {
            ...
            setTimeout(() => {
                ...
                const rawPayment = document.querySelector('input[name="paymentMethod"]:checked').value;
                ...
                else if (rawPayment === "wallets") paymentMethodName = "Wallets (Paytm/Amazon Pay)";
                ...
                let shipping = 0;
                const rawDelivery = document.querySelector('input[name="deliveryMethod"]:checked').value;
...
function setupConfirmationPage() {
    ...
    try {
        ...
        if (shippingEl) {
            shippingEl.textContent = order.shipping === 0 ? "Free" : `₹${order.shipping}`;
        }
...
function setupLoginPage() {
    ...
    if (forgotLink) {
        forgotLink.addEventListener("click", (e) => {
            ...
            if (email) {
                ...
                const enteredEmail = prompt("Please enter your registered email address to receive a reset link:");
                if (enteredEmail && enteredEmail.trim()) {
                    alert(`A password reset link has been simulated and sent to: ${enteredEmail.trim()}`);
                }
            }
        });
    }
    ...
    if (createLink) {
        createLink.addEventListener("click", (e) => {
            e.preventDefault();
            alert("Account creation is simulated. In a production environment, this would redirect to a registration page.");
        });
    }
    ...
    if (otpBtn) {
        otpBtn.addEventListener("click", () => {
            const mobile = prompt("Enter your mobile number to receive a one-time passcode:");
            if (mobile) {
...
function setupDashboardPage() {
    ...
    const dynamicOrderContainer = document.getElementById("dynamic-dashboard-order-row");
    ...
    const dbLeftArrow = document.querySelector(".dashboard-recommendations-section .arrow-left");
    const dbRightArrow = document.querySelector(".dashboard-recommendations-section .arrow-right");
...
const productDatabase = {
    makhana_cheese_onion: {
        ...
        benefits: ["Roasted, Not Fried", "Light & Crunchy", "Perfectly Seasoned", "Rich in Antioxidants", "Zero Trans Fat"],
    ...
    makhana_classic: {
        ...
        benefits: ["Roasted, Not Fried", "Light & Crunchy", "Classic Salted", "Rich in Antioxidants", "Zero Cholesterol"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    makhana_periperi: {
        ...
        benefits: ["Roasted, Not Fried", "Light & Crunchy", "Fiery Peri Peri", "Boosts Metabolism", "Zero Trans Fat"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    makhana_himalayan: {
        ...
        benefits: ["Roasted, Not Fried", "Mineral Rich Himalayan Salt", "Crunchy & Light", "Perfect Daily Snack", "Zero Trans Fat"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    almonds_california: {
        ...
        benefits: ["Double Sorted", "High in Vitamin E", "Rich in Protein", "Premium California Origin", "Boosts Brain Health"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    cashews_roasted: {
        ...
        benefits: ["Slow Roasted", "Rich & Creamy", "Lightly Salted", "Healthy Monounsaturated Fats", "Handpicked Quality"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    pistachios_roasted: {
        ...
        benefits: ["Easy to Shell", "Lightly Roasted", "Rich in Antioxidants", "Highly Nutritious", "100% Natural Processed"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    raisins_premium: {
        ...
        benefits: ["Naturally Sun Dried", "Zero Added Sugar", "Rich in Iron & Fiber", "Sweet & Juicy", "Ideal Dessert Topping"],
        benefitsImage: "images/makhana_bowl_love.png",
        specs: {
            "Brand": "Rein Oro",
    ...
    gift_box_premium: {
        ...
        benefits: ["Exquisite Festive Packaging", "Gold Embossed Box", "Curated Wellness Selection", "Perfect for Corporate Gifting", "Vacuum Packed Freshness"],
...
function setupProductDetails() {
    ...
    if (product) {
        ...
        if (categoryCrumbEl) {
            ...
            categoryCrumbEl.setAttribute("href", `shop.html?category=${encodeURIComponent(product.name)}`);
        }
        ...
        if (thumbnails.length > 0) {
            ...
            thumbnails[1].querySelector("img").setAttribute("src", product.benefitsImage);
            
            // Thumbnail 3 close up
            if (product.image.includes("makhana")) {
                thumbnails[2].setAttribute("data-image", "images/makhana_classic.png");
                thumbnails[2].querySelector("img").setAttribute("src", "images/makhana_classic.png");
            } else {
                thumbnails[2].setAttribute("data-image", "images/almonds_california.png");
                thumbnails[2].querySelector("img").setAttribute("src", "images/almonds_california.png");
            }
            ...
            thumbnails[4].querySelector("img").setAttribute("src", product.benefitsImage);
        }
        ...
        // Nutrition Table
        const nutritionTableBody = document.querySelector("#nutrition-table-el tbody");
    ...
    const addToCartAction = document.querySelector(".btn-detail-primary.add-to-cart-action");
